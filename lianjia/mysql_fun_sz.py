...
def insert_partition():
    ...
    # 执行sql语句
    sql = 'INSERT INTO partitions_sz (partition_name, partition_url) VALUES (%s,%s)'
    rows = cursor.execute(sql, ('4', 'qzcssz4'))
...
def insert_batch_apartment(apartment_list):
    if len(apartment_list)==0:
        ...
        cursor = conn.cursor()
        sql = 'INSERT INTO apartment_sz (detail_url, summary, partition_url) VALUES (%s, %s, %s)'
...
def select_apartments():
    ...
    cursor = conn.cursor()
    sql = 'select detail_url from apartment_sz where fangwuhuxing is null limit 0, 1000'
    cursor.execute(sql)
    conn.commit()
    cursor.close()
    conn.close()
